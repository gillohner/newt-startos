...
export const configurePangolin = sdk.Action.withInput(
  ...
  async ({ effects, input }) => {
    ...
    return {
      ...
      message:
        `Successfully configured Newt to connect to ${input.endpoint}. The service is restarting - check the service logs to verify the connection is successful. Look for "Tunnel connection to server established successfully!" or connection error messages.`,
      result: null,
    };
  },
);
